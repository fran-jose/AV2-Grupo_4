...
def run_GameOfLifeModel(width, height, cell_size, revive_probabilities, survival_probabilities, initial_config = None, colors = {"empty" : (0,0,0),
                                                                                   "filled" : (255, 255, 255)}):
    pygame.init()
    screen = pygame.display.set_mode((width * cell_size, height * cell_size))
    clock = pygame.time.Clock()

    model = GameOfLifeModel(width, height, revive_probabilities, survival_probabilities, alive_fraction = 0.2)
    running = True
...
run_GameOfLifeModel(200, 100, 5, {0:0.001, 3:1.0}, {2:1, 3:1})